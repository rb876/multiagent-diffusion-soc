...
class CondUNetModel(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.epsilon_based = epsilon_based
        ...
        self.log_likelihood_scaling = nn.Sequential(
            ...
            linear(time_embed_dim, time_embed_dim),
            nn.SiLU(),
            linear(time_embed_dim, 1),
        )

        self.log_likelihood_scaling[-1].weight.data.fill_(0.0)
        self.log_likelihood_scaling[-1].bias.data.fill_(init_scaling_bias)
    ...
    def get_time_scaling(self, timesteps):
        t_emb = timestep_embedding(timesteps, self.model_channels)

        time_scaling = self.log_likelihood_scaling(t_emb)[:, 0]
        return time_scaling
    ...
    def forward(self, x, timesteps):
        ...
        if self.epsilon_based:
            time_scaling = -time_scaling

        if self.use_residual:
            if self.in_channels == 15:
                return h + time_scaling[:, None, None, None] * x[:, 12:, :, :]
            if self.in_channels == 12:
                return h + time_scaling[:, None, None, None] * x[:, 9:, :, :]
            if self.in_channels == 9:
                return h + time_scaling[:, None, None, None] * x[:, 6:, :, :]
