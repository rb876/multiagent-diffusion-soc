...
def train_control_bptt(
    ...
    """
    Single training step for multi-agent control policies via BPTT under a stochastic optimal control (SOC) objective.
    NOTE: Optimizer step is within this function.
    """
    ...
    for t_idx in range(len(time_steps) - 1):
        ...
        scores = {}
        for key in agent_keys:
            # This will need to be modified as we expect to have different score models per agent in the future.
...
def fictitious_train_control_bptt(
    ...
    def _train_single_control_policy(player_key):
        ...
        for t_idx in range(len(time_steps) - 1):
            ...
            scores = {}
            for key in agent_keys:
                # we detach the score for other agents to prevent gradients flowing into their score computations, as we only want gradients w.r.t. the active player's control policy parameters.
                # By doing so, the Xˆ{i}_t doesn't depend on the other agents's states through the score model Xˆ{i}_{t-1}, 
                # and we only get gradients w.r.t. the active player's control policy parameters during backpropagation.
                if key == player_key:
